...
"""把这座城画出来：受光面与背光面、窗格（白天映天 / 夜里亮灯）、屋顶构件、
楼与楼之间的互相遮挡，以及建筑投在路面上的影子。

每一栋楼都是"有体积的"：影子按太阳方位斜过去、只铺到真实影长该到的地方（做过
一版通到底的竖条，一排楼排下来就是路上一道一道的明暗竖带，见 AGENTS.md §3.6）。
"""
...
from .model import (TAU, Building, Layers, Light, _dim, _far_color, _grad,
                    _hash01, _lit_window_colors, _norm, _pick_pal, _sky_mix,
                    clamp, wall_color)
