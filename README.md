...
- 有网络时**镜像真实天气**：外面下雨，窗上就下雨；外面阴天，太阳只剩一个隐约的亮斑；
  起雾时远处的屋顶真的会糊掉。**下多少雨也是真的**：雨丝的疏密、长短、快慢、
  有没有那层雨幕都按降水量（mm/时）来——毛毛雨是飘着几丝、细得几乎看不见，
  暴雨才是满窗的长划子，雨点砸在湿马路上会溅开小涟漪；
...
| **此刻的事实** | 左上角那张卡（空格收起/展开）分三层：**主角**是此刻的天气（大字温度、一句话天气，右边一行小字写体感 / 云量 / 今日高低温），**指标格**是太阳、日出、日落、月亮（两列，点了跳到那一刻去看），**小字条**是湿度、风、降水、能见度、紫外线、气压、露点。点主角或任一小字条摊开完整数据；右下角刷新立刻重问一次真实天气（成功还是失败都会回一句话）。底下那行写清楚**这份天气是什么时候问回来的**。|
| **场景能挑** | 菜单 → **场景**：行人、车辆、行道树、路灯、偶尔飞过的飞机、城市天际线、天上的云、星空、雨雪与闪电、窗台那盆植物、植物随风摇曳，每一项都能单独关掉（记住你的选择）；另有「全部画出来」与「只看天空（收起街上的一切）」两个一键。|
| **看着顺** | 画面由帧时钟驱动，默认 60 帧/秒（以前固定 10-14 帧，窗口没有焦点时只有 2-3 帧）。菜单 → 显示 → **画面流畅度** 可以按机器挑 24 / 30 / 45 / 60 / 120。|
...
| **把此刻留下来** | 菜单 → **把这扇窗存成图片…**：把现在这一帧（连信息卡与长卷）写成 `~/Pictures` 里的一个 PNG——画的就是你看到的那一帧，同名的绝不覆盖。预览时存下来的就是**预览那一刻**。纯本地，不出网。|
...
| 菜单 → 场景 → **跳到某天某时…** | 用日历 + 时/分选框直接跳到任意一天任意一刻 |
| 点信息卡上的 `日出 / 日落 / 月亮 / 太阳` | 跳到那一刻去看看（按 `Esc` 回到此刻）|
| 点信息卡上的主角块 / 任一小字条 | 摊开完整的一份天气：体感、露点、气压、紫外线、能见度、数据来源与更新时间 |
| 菜单 → 场景 | 挑窗外画什么（行人 / 车辆 / 行道树 / 云 / 星空 / 盆栽…），或一键「只看天空」 |
| 点信息卡右上角的箭头（或按 `C`） | 把卡片收成一条，只留时间与那句话（壁纸上的那张可以单独设成"跟随窗口 / 精简 / 完整"）|
| 点信息卡右下角的刷新（或按 `R`） | 立刻重问一次真实天气，问完会说明结果与更新时间 |
| 日弧上悬停 / 点一下 | 那条轨道铺的是**一整天**（只有日出到日落那一段是亮的）：看那一刻是几点，点一下跳过去 |
| `空格` | 显示 / 隐藏「此刻的事实」 |
| `C` / `R` | 收起信息卡 / 立刻刷新天气 |
| `F11` | 沉浸全屏 |
| 菜单 → 显示 → **画面流畅度** | 24 / 30 / 45 / 60（默认）/ 120 帧每秒；窗口没拿到焦点时按最多 30 帧省电 |
| 菜单 → **把这扇窗存成图片…** | 把此刻这一帧写成 `~/Pictures/chuang-日期-时刻.png`（同名不覆盖，点提示条可复制完整路径）|
| 右上角图钉 | 窗口置顶（缩窄后放在角落当"电子窗景"）|
| 右上角菜单 | 顶层按"这件事管什么"分：最常用的五件事（换一扇窗 / 跟随真实天气 / 沉浸全屏 / 窗口置顶 / 存成图片）、**场景 ▸**、**桌面壁纸 ▸**、**显示 ▸**（信息卡 / 长卷 / 跳到某天某时 / 画面流畅度）、**开机与关窗 ▸**、**关于与帮助 ▸**、退出。托盘里的菜单一模一样 |
...
桌面上的那张卡**不画能点的东西**（那里没有鼠标）：没有收起箭头、没有刷新按钮、
每行右边也没有小箭头。长卷和它下面那行整点标签会按**屏幕真实的可用区域**往上让，
不会被底部常驻的 dock / 面板压住。
...
（默认 60 帧/秒，菜单 → 显示 → 画面流畅度 可调），所以那里的人车走得顺滑；
...
嫌吵就在菜单 → 显示 → **画面流畅度** 里降到 24 或 30 —— 帧率的开销是线性的。
...
代码分层（`chuang/`）。1.2.0 把几个大文件按"一摊一件事"拆成了包：包的
`__init__.py` 把原来的名字原样再导出去，所以 `from chuang.render import SkyPainter`、
`street.roster`、`Weather(...)` 这些用法都没变；**任何一个模块都不许超过 700 行**
（`tests/test_project.py` 里有一条盯着）：
...
| `weather/` | 真实天气：`codes`（天气码与雨量）／`model`（那份数据与缓存）／`net`（只碰网络）／`service`（后台线程与限流）／`const` |
| `render/` | 全部 Cairo 绘制，按画面里的层分 12 个模块：`paint`（字与图标）／`sky`／`ground`／`sill`（窗台与盆栽）／`ribbon`（长卷）／`card`＋`cardpaint`（信息卡）／`notify`（提示条）／`painter`（组装与一帧的顺序）… |
| `city/` | 远山／远景楼群／近景建筑：`model` 长出这座城（种子＝城市名＋经纬度），`draw` 把它画出来（受光面、窗格、互相遮挡、路面上的影子） |
| `street/` | 街上的人与车：`actors`（谁在街上、几点最热闹）／`people`／`vehicles`／`props`（行道树与路灯）／`shadows`／`draw` |
...
| `pointer.py` | 鼠标：命中信息卡、拖长卷做时间旅行、滚轮微调 |
| `frames.py` | 帧率：谁来驱动重绘（帧时钟）、菜单里那几档怎么调 |
| `export.py` | 「把这扇窗存成图片」：命名、不覆盖、画一帧写盘（纯函数） |
| `app.py` | 窗口本身：画面、心跳、动作表、托盘与生命周期（壳在 `chrome.py`、单实例锁在 `instance.py`、置顶在 `topmost.py`、调试环境变量在 `devhooks.py`、关于窗在 `about.py`） |
